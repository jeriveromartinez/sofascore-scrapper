...
func TestSchemaTournament(t *testing.T) {
	s, err := schema.Parse(&Tournament{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaTeam(t *testing.T) {
	s, err := schema.Parse(&Team{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
	if s.Table != "teams" {
...
func TestSchemaPlaybackLog(t *testing.T) {
	s, err := schema.Parse(&PlaybackLog{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaApkVersion(t *testing.T) {
	s, err := schema.Parse(&ApkVersion{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaDomain(t *testing.T) {
	s, err := schema.Parse(&Domain{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
	if s.Table != "domains" {
...
func TestSchemaSofaScoreEvent(t *testing.T) {
	s, err := schema.Parse(&SofaScoreEvent{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
	if s.Table != "events" {
	...
	if eventID == nil {
		t.Error("expected field 'SofaScoreEventId' in SofaScoreEvent")
...
func TestSchemaDeviceTournament(t *testing.T) {
	s, err := schema.Parse(&DeviceTournament{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaGlobalTournamentConfig(t *testing.T) {
	s, err := schema.Parse(&GlobalTournamentConfig{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaRefreshToken(t *testing.T) {
	s, err := schema.Parse(&RefreshToken{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaContentStat(t *testing.T) {
	s, err := schema.Parse(&ContentStat{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
...
func TestSchemaCrashReport(t *testing.T) {
	s, err := schema.Parse(&CrashReport{}, &sync.Map{}, schema.NamingStrategy{})
	if err != nil {
		t.Fatalf("schema.Parse failed: %v", err)
	}
